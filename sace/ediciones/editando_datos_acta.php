...
if ( ($continuar) OR ($cancelar) )
{
	...
	if ( (! $error_ci_profesor) AND (! $error_fecha_aprobacion) )
	{
		...
		$sqlcmd = "UPDATE registro_actas SET "
				. "cedula_profesor='$_ci_profesor', fecha_aprobacion='$fecha_db', fecha_modificacion=NOW(), operador_modificacion='$PHP_AUTH_USER', "
				. "host_modificacion='$REMOTE_ADDR' "
				. "WHERE codacta='$_codacta' ";
		...
		if ($fecha_db == '0')
		{
			$sqlcmd = "UPDATE registro_actas SET fecha_aprobacion=NULL WHERE codacta='$_codacta' ";
		...
		if ($_ci_profesor == '0')
		{
			$sqlcmd = "UPDATE registro_actas SET cedula_profesor=NULL WHERE codacta='$_codacta' ";
...
/*
		...
		CREATE TABLE ediciones_actas (
		  eaid int(6) unsigned NOT NULL auto_increment,
		  transaccion_id int(6) unsigned,
...
*/
		if ($_ci_profesor == '0')	$_ci_profesor = 'NULL';
		...
		if ( ($fecha_db != $fecha_aprobacion_anterior) AND (! ( ($fecha_db == 'NULL') AND ($fecha_aprobacion_anterior == '0-0-0') ) ) )
		{

			if ($fecha_aprobacion_anterior == '0-0-0')	$fecha_aprobacion_anterior = 'NULL';


			$sqlcmd = "INSERT INTO ediciones_actas (codcohorte, codacta, fecha_modificacion, operador_modificacion, host_modificacion, operacion) VALUES ("
					. "'$_codcohorte', '$_codacta', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR', "
			...
			$sqlcmd = "UPDATE ediciones_actas SET transaccion_id='$eaid' WHERE eaid='$eaid' ";
		...
		if ($cedula_profesor_anterior == '')	$cedula_profesor_anterior = 'NULL';

		if ( ($_ci_profesor != $cedula_profesor_anterior) AND ($agregue_fecha) AND (! ( ($_ci_profesor == 'NULL') AND ($cedula_profesor_anterior == 'NULL') ) ) )
		{
	
			$sqlcmd = "INSERT INTO ediciones_actas (transaccion_id, codcohorte, codacta, fecha_modificacion, operador_modificacion, host_modificacion, operacion) VALUES ("
					. "'$eaid', '$_codcohorte', '$_codacta', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR', "
					. "'Se modifico la cedula del profesor del acta: $_codacta. La anterior era: $cedula_profesor_anterior, la nueva es: $_ci_profesor.') ";
...
<?
	$sqlcmd = "SELECT registro_actas.codasig, pensum_estudios.asignatura "
			. "FROM registro_actas, pensum_estudios "
			. "WHERE registro_actas.codasig=pensum_estudios.codasig AND registro_actas.codacta='$_codacta'";
...
<FORM ACTION="editando_datos_acta.php" METHOD="post">

<?
	$sqlcmd = "SELECT cedula_profesor, fecha_aprobacion FROM registro_actas where codacta='$_codacta'";
	...
	$cedula_profesor_anterior = $cedula_profesor;
...
<TR>
	...
	<TD WIDTH="300" ALIGN="left" VALIGN="top">
		<?
			...
			if ($cedula_profesor == 0) $cedula_profesor = '';

			if (! $_ci_profesor)
			{
				echo '<INPUT TYPE="text" NAME="_ci_profesor" VALUE="' . $cedula_profesor . '" SIZE="10" MAXLENGTH="9"> &nbsp;';

			} else {

				echo '<INPUT TYPE="text" NAME="_ci_profesor" VALUE="' . $_ci_profesor . '" SIZE="10" MAXLENGTH="9"> &nbsp;';
			}
		?>

		<FONT SIZE="-1" FACE="Verdana,Arial,Geneva">
			<? echo ucwords($apellidos_nombres) ?>
		</FONT>
...
<INPUT TYPE="hidden" NAME="cedula_profesor_anterior" VALUE="<? echo $cedula_profesor_anterior ?>">
...
?>
		...
		<FONT SIZE="-1" FACE="Verdana,Arial,Geneva">
			<B>
			Presione
				<A HREF="editar_calificaciones.php?codacta=<? echo $_codacta ?>"> 
