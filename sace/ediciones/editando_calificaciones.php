...
if ( ($continuar) OR ($cancelar) )
{
	...
	if ($cancelar)
	{
		$url = "editar_calificaciones.php?codacta=" . $codacta;
	...
	if ( (! $error_cedula) AND (! $error_calificacion) )
	{
		...
		if ($_cedula == $_cedula_old)
		{
			...
			$sqlcmd = "INSERT INTO ediciones_notas (codcohorte, codacta, fecha_modificacion, operador_modificacion, host_modificacion, operacion) VALUES ("
					. "'$_codcohorte', '$codacta', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR', "
			...
			$url = "editando_calificaciones_finalizado.php?codacta=$codacta";
			...
			if ($encontrado > 0)
			{
				...
				$sqlcmd = "UPDATE record_notas SET "
						...
						. "host_modificacion='$REMOTE_ADDR' "
						. "WHERE codacta='$codacta' AND cedula='$_cedula_old' ";
				...
				if ($_calificacion_old != $_calificacion)
				{
					...
					$sqlcmd = "INSERT INTO ediciones_notas (transaccion_id, codcohorte, codacta, fecha_modificacion, operador_modificacion, host_modificacion, operacion) VALUES ("
							. "'$enid', '$_codcohorte', '$codacta', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR', "
...
<FORM ACTION="editando_calificaciones.php" METHOD="post">
