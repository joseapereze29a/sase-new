...
$sqlcmd = "SELECT dc.codsede, dc.modalidad, dc.ciudad, dc.edo_prov, oe.codopest, oe.tipo, oe.mencion_especialidad, "
		...
		. "WHERE dc.codsede=oe.codsede AND dc.codsede=co.codsede AND oe.codopest=co.codopest ";
...
#echo "alora<BR>";		
		if ( ($datosPersonales->nombres) && ($datosPersonales->nombres != "") && ($datosPersonales->nombres != null) ) $nombres = 
$datosPersonales->nombres;
		if ( ($datosPersonales->apellidos) && ($datosPersonales->apellidos != "") && ($datosPersonales->apellidos != null) ) $apellidos = 
...
function buscarIndiceAcademicoDeEstudiante ( $registro, $cedula )
{
	...
	$sqlcmd1 = "SELECT codasig, asignatura, creditos, periodos, codasig_imp "
			 ...
			 . "WHERE codsede='" . $registro->codsede . "' AND pensum_estudios.codopest='" . $registro->codopest . "' AND 
pensum_estudios.status='Activa' "
...
#echo "nu: " . number_format(($notas/$total_creditos), 2, ',', '') . "<BR>";
