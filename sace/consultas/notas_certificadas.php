...
$stmt1 = mysqli_prepare($conexion, $sql1)
    or exit('Error prepare datos personales: '.mysqli_error($conexion));
...
if (mysqli_stmt_fetch($stmt1)) {
    ...
    if ($nombres_db && $apellidos_db) {
        $apellidos_nombres = mb_convert_case($nombres_db, MB_CASE_TITLE, 'UTF-8')
                           . ' '
    ...
    elseif ($apellidos_db) {
        $apellidos_nombres = mb_convert_case($apellidos_db, MB_CASE_TITLE, 'UTF-8');
    }
    else {
    ...
    if (empty($fecha_nacimiento_db) || $fecha_nacimiento_db === '0000-00-00') {
        $fecha_nacimiento = 'No Existe Registro';
    } else {
        $fecha_nacimiento = date('d/m/Y', strtotime($fecha_nacimiento_db));
    }
} else {
    // No hay registro para esa cédula
    $apellidos_nombres = 'No Existe Registro';
    $nacionalidad      = '--';
    $fecha_nacimiento  = 'No Existe Registro';
}
...
// 3) CONSULTA DATOS DE COHORTE Y PROGRAMA
$sql2 = "
  SELECT
    dc.ciudad,
    oe.tipo,
...
$stmt2 = mysqli_prepare($conexion, $sql2)
    or exit('Error prepare cohortes: '.mysqli_error($conexion));
...
if (mysqli_stmt_fetch($stmt2)) {
    ...
    if (empty($fecha_inicio_db) || $fecha_inicio_db === '0000') {
        $fecha_inicio = 'No Existe Registro';
    } else {
        // Mantener sólo el año o formatear como necesites
...
// Ahora dispones de todas estas variables:
//   $apellidos_nombres, $nacionalidad, $fecha_nacimiento,
//   $ciudad, $tipo, $mencion_especialidad, $codopest, $codsede,
//   $titulo_a_otorgar, $periodos, $creditos, $fecha_inicio,
//   $periodo_lectivo

### Busco los Datos Basicos Personales en la Base de Datos
/*$sqlcmd = "SELECT apellidos, nombres, nacionalidad, fecha_nacimiento "
		. "FROM datos_personales "
		. "WHERE cedula='$cedula' ";
...
while ($registro = mysql_fetch_object($query))
{
	...
	//if($nacionalidad !== 'Venezolana' or $nacionalidad !== 'Extranjera'){
	//	$nacionalidad = '--';
	//}
}
...
$sqlcmd = "SELECT directorio_cippsv.ciudad, oportunidades_estudio.tipo, oportunidades_estudio.mencion_especialidad, "
		. "oportunidades_estudio.codopest, oportunidades_estudio.codsede, oportunidades_estudio.titulo_a_otorgar, "
...
while ($registro = mysql_fetch_object($query))
{
	...
	$titulo_a_otorgar = $registro->titulo_a_otorgar;
...
<TR>
	...
	<TD WIDTH="300" ALIGN="left" VALIGN="top">
		<FONT SIZE="-1" FACE="Arial">
			<B>C.I. No.:</B> <?php echo $nacionalidad; ?> <? echo strtr (number_format($cedula), ",", ".") ?>
		</FONT>
	</TD>
</TR>
<TR>
...
// 2) Preparar una sola consulta que devuelve PARA CADA ASIGNATURA
//    su código, nombre, créditos, periodo y **todas** sus calificaciones
$sql = "
  SELECT
    pe.codasig,
    pe.asignatura,
...
if (!$stmt) {
    die('Prepare falló: '.mysqli_error($conexion));
}

// Los placeholders (?) van en orden: cedula, cohorte, cedula, cohorte, codsede, codopest
...
// 3) Asignar columnas del resultado
mysqli_stmt_bind_result(
    $stmt,
    $codasig,
    $asignatura,
...
// 4) Procesar las filas, alternar color y acumular promedio ponderado
$mapCalif = array(
    404 => 'No Cursó',
     99 => 'Reprobado',
    100 => 'Aprobado',
...
while (mysqli_stmt_fetch($stmt)) {
    // 4.1 Determinar texto de la calificación
    if ($calRaw === null) {
        $calif = 'Pendiente';
    }
    elseif (isset($mapCalif[$calRaw])) {
        $calif = $mapCalif[$calRaw];
    }
    else {
        $calif = $calRaw;
    }

    // 4.2 Acumular ponderado sólo si es nota numérica válida
    if (is_numeric($calRaw) && $calRaw >= 1 && $calRaw <= 20 && $creditos > 0) {
        $totalPonderado += $calRaw * $creditos;
        $totalCreditos  += $creditos;
    }

    // 4.3 Alternar color de fondo
...
// 5) Calcular y mostrar promedio ponderado
$promedio = ($totalCreditos > 0)
          ? round($totalPonderado / $totalCreditos, 2)
          : 0;

//echo "<p><strong>Promedio ponderado:</strong> {$promedio}</p>";
...
Jefe de Control de Estudios
	...
	<TD WIDTH="340" ALIGN="center" VALIGN="top">
		...
		<TR>
			<TD WIDTH="340" ALIGN="center" VALIGN="top">
				<FONT SIZE="-1" FACE="Arial">
					<B>Indice Acad&eacute;mico: <? echo number_format(($totalPonderado/$totalCreditos), 2, ',', '')  ?></B><BR>
				</FONT>
