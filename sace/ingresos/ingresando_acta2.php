...
if ( ($continuar_f) OR ($continuar_a) )
{
	...
	if (! $error_1)
	{
		...
		for ($i=1; $i<21; $i++)
		{
			...
			if ($$cedula)
			{
					#if (	( ($$nota) AND  ( ($$nota < 1) OR ($$nota > 20) )	)	)#OR	(  ($$calificacion) AND ( ($$calificacion < 1) OR ($$calificacion > 404) ) )	)	)
	...
	if ( (! $error_1) AND (! $error_2) AND (! $error_6) )
	{
	...
	if ( (! $error_1) AND (! $error_2) AND (! $error_6) AND (! $error_7) )
	{
			for ($i=1; $i<21; $i++)
			{
				$iguales = 0;
	...
	if ( (! $error_1) AND (! $error_2) AND (! $error_3) AND (! $error_6) AND (! $error_7) )
	{
	...
	if ( (! $error_1) AND (! $error_2) AND (! $error_3) AND (! $error_6) AND (! $error_7) AND (! $error_5) )
	{
	...
	if ( (! $error_1) AND (! $error_2) AND (! $error_3) AND (! $error_6) AND (! $error_7) AND (! $error_4) AND (! $error_5) )
	{
				...
				if ($cantidad > 0)
				{
							...
							for ($i=1; $i<21; $i++)
							{
									...
									if ($$cedula)
									{
										...
										if (	( ($$nota >= 1) AND ($$nota < 21) ) OR ( ($$calificacion >= 1) AND ($$calificacion < 405) )		)
										{
											...
											$sqlcmd = "INSERT INTO record_notas (codacta, cedula, calificacion, fecha_creacion, operador_creacion, host_creacion) VALUES ("
													. "'$query_codacta', '" . $$cedula . "', '" . $meto_calif . "', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR')";
							...
							if ($continuar_a)
							{		
									$url = "ingresando_acta.php?_codcohorte=" . $_codcohorte . "&_codasig=" . $_codasig . "&_codacta=" . $_codacta . "&_ok=1";
							...
							if ($_cd) $_codacta_final = $_codacta_final . "CD";
							
							if ($_ci_profesor == "") $_ci_profesor = 0;
							...
							$sqlcmd = "INSERT INTO registro_actas (codcohorte, codasig, codacta, cedula_profesor, fecha_aprobacion, fecha_creacion, "
									. "operador_creacion, host_creacion) VALUES ("
									. "'$_codcohorte', '$_codasig', '$_codacta_final', '$_ci_profesor', '$_fecha_aprobacion', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR')";
							...
							if ($_fecha_aprobacion == '0000-00-00')
							{
								...
								$sqlcmd = "UPDATE registro_actas SET fecha_aprobacion=NULL WHERE codacta='$_codacta_final' ";
							...
							if ($_ci_profesor == 0)
							{
								...
								$sqlcmd = "UPDATE registro_actas SET cedula_profesor=NULL WHERE codacta='$_codacta_final' ";
							...
							if ($_ci_profesor == 0) $_ci_profesor = "";
							...
							for ($i=1; $i<21; $i++)
							{
									...
									if ($$cedula)
									{
										...
										if (	( ($$nota >= 1) AND ($$nota < 21) ) OR ( ($$calificacion >= 1) AND ($$calificacion < 405) )		)
										{
												...
												$sqlcmd = "INSERT INTO record_notas (codacta, cedula, calificacion, fecha_creacion, operador_creacion, host_creacion) VALUES ("
														. "'$_codacta_final', '" . $$cedula . "', '" . $meto_calif . "', NOW(), '$PHP_AUTH_USER', '$REMOTE_ADDR')";
		...
		<TR>
			<TD WIDTH="400" ALIGN="left" VALIGN="top">
				<FONT SIZE="-1" FACE="Verdana,Arial,Geneva">
					<A HREF="../">Home</A>: <A HREF="seleccion_de_sede.php">Selecci&oacute;n de Sede</A>:
					<A HREF="seleccion_cohorte.php?_codsede=<? echo $codsede ?>&_codopest=<? echo $codopest ?>">Cohortes Existentes</A>:<BR>
					<A HREF="seleccion_acta.php?_codcohorte=<? echo $_codcohorte ?>">Actas Existentes</A>: 
					<A HREF="ingreso_acta.php?_codcohorte=<? echo $_codcohorte ?>">Acta Selecci&oacute;n</A>: Ingreso de Notas
				</FONT>	
			</TD>
		</TR>
		</TABLE>
...
<FORM ACTION="ingresando_acta.php" METHOD="post">
...
<?
	...
	if ($cantidad > 0)
	{
		...
		$sqlcmd = "SELECT fecha_aprobacion FROM registro_actas where codacta='$_codacta' ";
		...
		$sqlcmd = "SELECT profesores_cippsv.apellidos_nombres "
				. "FROM profesores_cippsv, registro_actas "
				. "WHERE profesores_cippsv.cedula_profesor=registro_actas.cedula_profesor AND registro_actas.codacta='$_codacta' ";
	...
	<TD WIDTH="300" ALIGN="left" VALIGN="top">
		...
		<?
			if ($acta_existe)
			{
				...
				if ( ($apellidos_nombres != NULL) AND ($apellidos_nombres != '') )
				{
					echo ucwords(strtolower($apellidos_nombres));
		...
		?>
				<INPUT TYPE="text" NAME="_ci_profesor" VALUE="<? echo $_ci_profesor ?>" SIZE="10" MAXLENGTH="9"> &nbsp;
...
if ( ($error_1) OR ($error_2) OR ($error_3) OR ($error_4) OR ($error_5) OR ($error_6) OR ($error_7) )
{
...
<FONT SIZE="-1" FACE="Verdana,Arial,Geneva">
	...
	<?
		if ($_codacta) :
	?>
			<A HREF="javascript:popup('_blank', 'detalle_acta.php?codacta=<? echo $_codacta ?>',650,600)">
...
<?
	if ($_cd) echo '<INPUT TYPE="hidden" NAME="_cd" VALUE="1">';
?>


<TABLE BORDER="0" WIDTH="450" CELLSPACING="2" CELLPADDING="2" BGCOLOR="#FF0000">
<TR>
