...
### Este Script Agrega Usuarios al Sistema, para te tengan Acceso a traves de REALMs
### El Script muestra una Forma, que luego que el Operador la llene, y la envie de vuelta
### al Servidor, el Script se encarga se verificar los Datos.
###
### Si los Datos estan correctos, los Agrega a la BD, despues los Agrega a los REALMs y
### por ultimo a el archivo que mantiene las Claves.
...
if ($continuar)
{
	...
	### Agregando a un 'User' dicho Usuario no puede existir en la BD para el momento.
	...
	if (! $user_no_valido)	### Verifico si el 'user' es Valido o No
	{
		...
		if ($cantidad_encontrada > 0) $user_existe_db = 1;		### Si el 'User' ya Existe en la BD, marco el error
	...
	### Si no hay Error, Inserto la Data en la BD y ademas en el REALMs, a traves del funcion 'accesos'

	if ( (! $user_no_valido) AND (! $user_existe_db) AND (! $pass_no_valido) )
	{
	
		### Agrego los Datos a la Base de Datos

		$sqlcmd = "INSERT INTO usuarios_sace (user, pass, usuario) VALUES ('$user', '$pass', '$usuario') ";

		$query = mysql_db_query(DB_DATABASE,"$sqlcmd");


		### Agrego los Datos a los Realm (htaccess)
		...
		for ($i=1; $i<$cantidad_modulos+1; $i++)
		{
			if ($$modulos[$i] == 'SI')
			{
				accesos($modulos[$i], "agregar", $user);	### Hago el llamado a la funcion 'accesos'
			}

		}


		### Agrego la Clave, en el archivo de Claves 'passwords'
...
<TR>
	...
	<TD ALIGN="left" WIDTH="300" VALIGN="top">
		<INPUT TYPE="text" NAME="user" VALUE="<? echo $user ?>" SIZE="18" MAXLENGTH="16">
	</TD>
...
<TR>
	...
	<TD ALIGN="left" WIDTH="300" VALIGN="top">
		<INPUT TYPE="text" NAME="pass" VALUE="<? echo $pass ?>" SIZE="18" MAXLENGTH="16">
	</TD>
	<TD ALIGN="left" WIDTH="250" VALIGN="top">
...
<?
	...
	if ( ($user_no_valido) OR ($user_existe_db) OR ($pass_no_valido) )
	{
		...
		{
			echo '<FONT SIZE="-1" FACE="Verdana,Arial,Geneva">';
			echo '&bull; El Usuario que esta intentando Ingresar, ya existe, favor revisar.';
			echo '</FONT><BR>';
		}
